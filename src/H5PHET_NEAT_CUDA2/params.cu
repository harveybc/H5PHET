...
#include "params.h"
//tamaño de la población y número de cuda cores (mp*8)
#define THREADSPB 96
    ...
    while ((argc > 1) && (argv[1][0] == '-'))
    {
        switch (argv[1][1])
        {
            ...
            case 'x':
                ...
                if (conf->cargarDistrib==0) conf->cargarDistrib=2;
                if (conf->cargarNNP==0) conf->cargarNNP=conf->spEspecies;
            break;
...
//      VERIFICAR que lso nombres globales no correspondan con nombres de variables locales
    ...
    conf->cargarNNP=0; //(8) número máximo de genomas a cargar desde NNP después de alcanzar spEspecies
    conf->probInterSp = 0.001; // (0.001) probabilid por genoma de que se produzca un cruce entre especies diferentes
    ...
    conf->sizePob = THREADSPB; 			//(96)Tamaño de la población.
    conf->spEspecies = conf->sizePob/12; //(8)Número de especies (12 genomas por especie)
    ...
    conf->probMutTh = 0.03; //Probabilidad de mutación de threshold en cada genoma.
    conf->probMutAN = 0.01;	//(0.01) Probabilidad de mutación AN durante ciclo principal
    // razon de complejidad nodos/conex
    conf->probMutAC = 0.01; //(0.01)Probabilidad de mutación AC durante ciclo principal
    conf->threshold = 0.7; //Thrreshold de compatibilidad de especies usado para diferenciar entre una especie y otra
