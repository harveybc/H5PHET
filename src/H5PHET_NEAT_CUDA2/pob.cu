...
__global__ void evalPobCUDA(int numEntradas, int numSalidas, int numBias, int numDatos, float* dataGTDf, int** conexIn, int** conexOut, float** conexPeso, int* tamListaConexPost, float** valorC, float** valorTr, float* fitness)
{
    ...
    float r_valorTr[4];
	// vector de tamaños de lista de conexiones.
	...
	// vector de datos de entrenamiento leidos en shared de tamaño THREADSPB
	__shared__ float s_dataGTDf[THREADSPB];
	// vector de fitness resultante para cada genoma por repetición entre grbaciones a global.
	__shared__ float s_fitness[THREADSPB];
    // vector de valor calculado para para calculo de fitness por correlación.
	__shared__ float s_valorC[THREADSPB];
    // vector de valor de entrenamiento para para calculo de fitness por correlación.
    ...
    for (i=0;i<numReps;i++)
    {
       ...
       // __syncthreads();
		...
		for (j=0;j<numRepsShared;j++)
        {
            ...
            // saca el Fsigma del último nodo.// OPTIMIZABLE: r_tamListaConexPost-1 puede ser una cte o reg
            ...
            // acumula para calculo de fitness con correlación
...
// retorna 0 si hubo error
{
    ...
    // inicializa vectores de evaluación.
    for (i=0;i<numGenomas;i++)
    {
        ...
        // reordena listaConexData para que estén primero los nodos necesarios y elimina enableds (optimización)
...
//TODO: dividir sc en selección y reproducción.
        ...
        // realiza perturbación de pesos 		VERIFICADA
        fclose(conf->logFile);
        conf->logFile=fopen(conf->fileNameLog,"a+");
...
//#pragma omp parallel for
        ...
        // introduce población desde archivo NNP en diferentes especies
...
*/
        ...
        guardarRepresentantesNNP(conf->fileNameNNPv1, conf);
//Todo: quitar cuando se ahay solucionado problama de delay en especiación
...
*/
        ...
        // CUDA: reserva memoria en device para valorC[sizePob] usado para calculo de fitness con correlación.
