...
int actualizarListaConex(int indexpob,TConfig* conf)
{
    int i=indexpob;
    //reserva memoria para ordenEval
    conf->ordenEval[indexpob]=(int*)malloc(conf->maxNodos*sizeof(int));
    if (!conf->ordenEval[indexpob])
    {
        printf("\nError 66.0.7 en actualizarListaConex() llamando a malloc()");
        return(0);
    }
    ...
    // libera memoria de ordenEval
    free(conf->ordenEval[indexpob]);
    return(1);
}
...
float evalGenom(int indexpob,TConfig* conf)
{
    ...
    float valor[2000]; // valor de salida de la neurona
  //  float valorAnt[2000]; // valor de salida de la neurona

    float acum;
    //reserva memoria para ordenEval
    conf->ordenEval[indexpob]=(int*)malloc(conf->pob[i].totalNodos*sizeof(int));
    if (!conf->ordenEval[indexpob])
    {
    ...
    // reordena listaConexData para que estén primero los nodos necesarios y elimina enableds (optimización)
    conf->tamListaConexPost[i]=ordenarListaConexF(conf->headerSNN[i], conf->listaConexData[i] ,conf->ordenEval[i],conf->tamOrdenEval[i]);
    // inicializa en 0 los elementos de la submatriz que se va a multiplicar. y del vector valor[]
...
//Retorna 0 si hay error, 1 si ok
    ...
    tmpFit=actualizarListaConex(indexpob,conf);
    ...
    {
        printf("ERROIR conf->tamListaConexPost[%d]==0",indexpob);
        //exit(0);
    }
...
//Retorna 0 si hay error, 1 si Ok
    ...
    *pGenomaDst=*pGenomaSrc;
    ...
    pGenomaDst->nodo=memcpy(pGenomaDst->nodo,pGenomaSrc->nodo, pGenomaSrc->totalNodos * ((unsigned  int)sizeof(GenNodoF)));
    ...
    pGenomaDst->conex=memcpy(pGenomaDst->conex,pGenomaSrc->conex, pGenomaSrc->totalConexiones * ((unsigned  int)sizeof(GenConexF)));
    return(1);
