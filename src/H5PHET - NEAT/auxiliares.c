...
float randL(TConfig* conf)
{
    ...
    conf->punteroRand=(*conf->punteroRand).next;
    return(valor);
}
...
// TODO: guardar después de evaalgenom para verificar y setear el leastfitness.
    ...
    for(j=0; j<conf->headerSNN[indexpob].numSalidas; j++)
    {
        ...
        recurOrganicer(temp+j, conf->headerSNN[indexpob] ,conf->listaConexData[indexpob],  conf->ordenEval[indexpob], valCalculado,&(conf->tamOrdenEval[indexpob]),conf);
    }
        ...
        if (listaConexData[i].conexOut==indexNodoOut)
            ...
            if ((listaConexData[i].enabled)&&(indexNodoOut!=headerSNN.numEntradas))
            {
                ...
                if (!valCalculado[listaConexData[i].conexIn])
                    ...
                    recurOrganicer(listaConexData[i].conexIn, headerSNN ,listaConexData, ordenEval, valCalculado, cont,conf);
            }
    }
    //retorna 1
    return(1);
...
//TODO: verificar rangos de entrada y salida de cada aprox sigma
    ...
    if (param==3)
    {
        return (fX>=0 ? fX>=1 ? 0: cos(M_PI*fX): fX<=-1 ? 0: fX+1) ;
...
// retorna 0 si hay error, 1 si OK
    ...
    for (i=0; i<conf->pob[index].totalNodos; i++)
    {
        if (i>0)
            if (conf->pob[index].nodo[i].innovNum==0)
            {
                fclose(conf->logFile);
                conf->logFile=fopen(conf->fileNameLog,"a+");
                fprintf(conf->logFile,"<br>\nError 1 en pob[%u] verificando innovNum de gen nodo[%u]!=0  es igual a 0 el maxInnovNumNodo",index,i);
                return(0);
            }
        if (conf->pob[index].nodo[i].innovNum>conf->pob[index].maxInnovNumNodo)
        {
            fclose(conf->logFile);
            conf->logFile=fopen(conf->fileNameLog,"a+");
            //fprintf(conf->logFile,"<br>\nError 2 en pob[%u] verificando no corresponde el maxInnovNumNodo[%i](%u,%d)",index,i,conf->pob[index].nodo[i].innovNum,conf->pob[index].maxInnovNumNodo);
            fprintf(conf->logFile,"Err2");
           // return(0);
        }
    }
    ...
    for (i=0; i<conf->pob[index].totalConexiones; i++)
    {
        ...
        if (conf->pob[index].conex[i].innovNum>conf->pob[index].maxInnovNumConex)
        {
            fclose(conf->logFile);
            conf->logFile=fopen(conf->fileNameLog,"a+");
            //fprintf(conf->logFile,"<br>\nError 3 en pob[%u] verificando no corresponde el maxInnovNumConex[%i](%i.%d)",index,i,conf->pob[index].conex[i].innovNum,conf->pob[index].maxInnovNumConex);
            fprintf(conf->logFile,"Err3");
            //return(0);
        }
    }
