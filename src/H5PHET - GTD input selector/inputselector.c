...
//      métodoPonderac = 0=son sumatoria de correlac de entradas en denominador o 1= con multiplicatoria.
{
    ...
    for (i=0;i<index;i++)
        if (metodoPonderac==0)
            ...
            mDn=mDn*fabs(correlac(s[index], i, buffer, correlacM, medias, numDatos));
    //obtiene el promedio de correlaciones entre las entradas
    if (index!=0)
        mDn=mDn/index;
    //optiene el promedio de correlaciones con salidas
    if (headerGTD.numSalidas>1)
        mUp=mUp/headerGTD.numSalidas;
    return(mUp/mDn); //siempre es positivo
}

int main()
{
    int numEntradasSelec=3; //Número de entradas con las que se generará el GTD V1.
        ...
        // calcula mDn= multip(correlac(index,entradasSeleccionadas)) es decir entre 0 y index
          for (j=0;j<header.numEntradas;j++)
            if (metodoPonderac==0)
                ...
                mDn=mDn*fabs(correlac(i, j, buffer, correlacM, medias, numDatos));
        //obtiene el promedio de correlaciones entre las entradas
        if (header.numEntradas!=0)
            mDn=mDn/(header.numEntradas);
//Modificado para mup2
        if (mDn!=0)
        {
